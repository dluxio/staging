        ...
        .then(b => {
            ...
            for (i in User.dex.queue) {
                if (User.opts.agent !== User.dex.queue[i]) {
                    ...
                    node.innerHTML = `<a href="#" onclick="insertBal('${User.dex.queue[i]}', 'custodialAgent', 'innerText');dexmodal(User.opts.pair,Useropts,type)">${User.dex.queue[i]} - Fee: .0DLUX - Trust: Hi - Liquid: ${parseInt(a[i].balance/1000)}</a>`
            ...
            for (i in User.dex.queue) {
                if (User.opts.to !== User.dex.queue[i]) {
                    var node = document.createElement('li')
                    node.innerHTML = `<a href="#" onclick="insertBal('${User.dex.queue[i]}', 'escrowAgent', 'innerText');dexmodal(User.opts.pair,Useropts,type)">${User.dex.queue[i]} - Fee: .0DLUX - Trust: Hi - Liquid: ${parseInt(a[i].balance/1000)}</a>`
